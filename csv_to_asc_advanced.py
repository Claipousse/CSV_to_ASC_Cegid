...
class CSVToASCConverter:
    def __init__(self, root):
        ...
        self.affil_gxo_file_path = tk.StringVar()
        self.affil_deret_file_path = tk.StringVar()
    ...
    def setup_ui(self):
        ...
        for i, (label, var, cmd) in enumerate([
            ...
            ("AFFIL GXO:", self.affil_gxo_file_path, self.select_affil_gxo_file),
            ("AFFIL DERET:", self.affil_deret_file_path, self.select_affil_deret_file),
...
⚠️ Important : L'ordre doit être exactement respecté, moins flexible que le filtrage par nom.""")
    ...
    def select_csv_file(self):
        ...
        if file_path:
            ...
            self.succ_gxo_file_path.set(f"{base_name}_SUCC_GXO.asc")
            self.succ_deret_file_path.set(f"{base_name}_SUCC_DERET.asc")
            self.affil_gxo_file_path.set(f"{base_name}_AFFIL_GXO.asc")
            self.affil_deret_file_path.set(f"{base_name}_AFFIL_DERET.asc")
            self.fra_gxo_file_path.set(f"{base_name}_FRA_GXO.asc")
            self.fra_deret_file_path.set(f"{base_name}_FRA_DERET.asc")
    ...
    def select_affil_gxo_file(self):
        self._select_file("Enregistrer le fichier AFFIL GXO", self.affil_gxo_file_path)
            
    def select_affil_deret_file(self):
        self._select_file("Enregistrer le fichier AFFIL DERET", self.affil_deret_file_path)
    ...
    def convert_file(self):
        try:
            required_files = [
                ...
                (self.affil_gxo_file_path.get(), "Veuillez spécifier un fichier de sortie AFFIL GXO"),
                (self.affil_deret_file_path.get(), "Veuillez spécifier un fichier de sortie AFFIL DERET"),
            ...
            self.log_message("Début de la conversion avec séparation SUCC/AFFIL/FRA et GXO/DERET...")
            ...
            df_valid[column_mapping['best']] = df_valid[column_mapping['best']].fillna('DERET').astype(str).str.strip()
            df_valid.loc[df_valid[column_mapping['best']].isin(['', 'nan']), column_mapping['best']] = 'DERET'
            ...
            df_affil_gxo = df_valid[(df_valid['store_type'] == 'AFFIL') & (df_valid[column_mapping['best']] == 'GXO')].copy()
            df_affil_deret = df_valid[(df_valid['store_type'] == 'AFFIL') & (df_valid[column_mapping['best']] != 'GXO')].copy()
            ...
            self.log_message(f"Lignes SUCC GXO: {len(df_succ_gxo)}")
            self.log_message(f"Lignes SUCC DERET: {len(df_succ_deret)}")
            self.log_message(f"Lignes AFFIL GXO: {len(df_affil_gxo)}")
            self.log_message(f"Lignes AFFIL DERET: {len(df_affil_deret)}")
            self.log_message(f"Lignes FRA GXO: {len(df_fra_gxo)}")
            self.log_message(f"Lignes FRA DERET: {len(df_fra_deret)}")
            
            results = {}
            for data, path, file_type in [
                (df_succ_gxo, self.succ_gxo_file_path.get(), "SUCC GXO"),
                (df_succ_deret, self.succ_deret_file_path.get(), "SUCC DERET"),
                (df_affil_gxo, self.affil_gxo_file_path.get(), "AFFIL GXO"),
                (df_affil_deret, self.affil_deret_file_path.get(), "AFFIL DERET"),
            ...
            messagebox.showinfo("Succès", 
                              ...
                              f"Fichier AFFIL GXO: {results['AFFIL GXO'][0]} magasins, {results['AFFIL GXO'][1]} lignes, {results['AFFIL GXO'][2]} pièces\n"
                              f"Fichier AFFIL DERET: {results['AFFIL DERET'][0]} magasins, {results['AFFIL DERET'][1]} lignes, {results['AFFIL DERET'][2]} pièces\n"
